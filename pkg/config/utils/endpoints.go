...
// getEndpoint implements the logic to extract the DD URL from a config, based on `site` and ddURLKey
func getEndpoint(c config.ConfigReader, prefix string, ddURLKey string) string {
...
func GetMainEndpoint(c config.ConfigReader, prefix string, ddURLKey string) string {
	return getEndpoint(c, prefix, ddURLKey)
}
...
func GetInfraEndpoint(c config.ConfigReader) string {
	return getEndpoint(c, infraURLPrefix, "dd_url")
}
